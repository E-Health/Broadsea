...
(
  select concept_id from vocabulary.CONCEPT where concept_id in (439459,40481022,40479553,440922,436583,440927,4214956,4015724)
UNION  select c.concept_id
  from vocabulary.CONCEPT c
  join vocabulary.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (440922,436583)
...
select C.person_id, C.observation_id as event_id, C.observation_date as start_date, (C.observation_date + 1*INTERVAL'1 day') as END_DATE, C.observation_concept_id as TARGET_CONCEPT_ID, C.visit_occurrence_id
from
(
  select o.*
  FROM synpuf5.OBSERVATION o
